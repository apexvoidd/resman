...
async def init_db() -> None:
    """Create tables and ensure missing columns are auto-added."""
        ...
        def _sync_migrations(sync_conn):
            ...
            if "guest_sessions" in inspector.get_table_names():
                ...
                if "is_locked" not in cols:
                    sync_conn.execute(
                        text(
                            f"ALTER TABLE guest_sessions ADD COLUMN is_locked BOOLEAN DEFAULT {bool_false} NOT NULL;"
                        )
                    )
                if "bill_requested_at" not in cols:
                    sync_conn.execute(
                        text(
                            f"ALTER TABLE guest_sessions ADD COLUMN bill_requested_at {ts_type};"
                        )
                    )
                if "can_submit_review" not in cols:
                    sync_conn.execute(
                        text(
                            f"ALTER TABLE guest_sessions ADD COLUMN can_submit_review BOOLEAN DEFAULT {bool_false} NOT NULL;"
                        )
                    )
                if "expires_at" not in cols:
                    if is_pg:
                        sync_conn.execute(
                            text(
                                "ALTER TABLE guest_sessions ADD COLUMN expires_at TIMESTAMP NOT NULL DEFAULT (now() + interval '2 hours');"
                            )
                        )
                    else:
                        sync_conn.execute(
                ...
                if "occupied_at" not in cols:
                    sync_conn.execute(
                        text(
                            f"ALTER TABLE guest_sessions ADD COLUMN occupied_at {ts_type};"
                        )
                    )
                if "verification_requested_at" not in cols:
                    sync_conn.execute(
                        text(
                            f"ALTER TABLE guest_sessions ADD COLUMN verification_requested_at {ts_type};"
                        )
                    )
                ...
                if "cooldown_until" not in cols:
                    sync_conn.execute(
                        text(
                            f"ALTER TABLE guest_sessions ADD COLUMN cooldown_until {ts_type};"
                        )
                    )
                ...
                if "reservation_expires_at" not in cols:
                    sync_conn.execute(
                        text(
                            f"ALTER TABLE guest_sessions ADD COLUMN reservation_expires_at {ts_type};"
                        )
                    )
            ...
            if "restaurant_settings" in inspector.get_table_names():
                ...
                if "is_closed" not in cols:
                    sync_conn.execute(
                        text(
                            f"ALTER TABLE restaurant_settings ADD COLUMN is_closed BOOLEAN DEFAULT {bool_false} NOT NULL;"
                        )
                    )
            ...
            if "reviews" in inspector.get_table_names():
                ...
                if "is_hidden" not in cols:
                    sync_conn.execute(
                        text(
                            f"ALTER TABLE reviews ADD COLUMN is_hidden BOOLEAN DEFAULT {bool_false} NOT NULL;"
                        )
                    )
                if "is_verified" not in cols:
                    sync_conn.execute(
                        text(
                            f"ALTER TABLE reviews ADD COLUMN is_verified BOOLEAN DEFAULT {bool_true} NOT NULL;"
                        )
                    )
