...
def upgrade() -> None:
    ...
    op.add_column("ingredients", sa.Column("version_id", sa.Integer(), server_default="1", nullable=False))
    op.create_index("ix_ingredients_is_active", "ingredients", ["is_active"])

    # ── stock_history table ─────────────────────────────────────────────────
    op.add_column("stock_history", sa.Column("previous_quantity", sa.Numeric(12, 3), server_default="0", nullable=False))
    op.add_column("stock_history", sa.Column("new_quantity", sa.Numeric(12, 3), server_default="0", nullable=False))
    op.add_column("stock_history", sa.Column("action_type", sa.String(50), server_default="adjustment", nullable=False))
    op.add_column("stock_history", sa.Column("invoice_number", sa.String(100), nullable=True))
    ...
    op.add_column("stock_history", sa.Column("recorded_by_user_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("users.id", ondelete="SET NULL"), nullable=True))
    op.create_index("ix_stock_history_action_type", "stock_history", ["action_type"])
    op.create_index("ix_stock_history_recorded_by_user_id", "stock_history", ["recorded_by_user_id"])

    # ── purchase_history table ──────────────────────────────────────────────
    op.add_column("purchase_history", sa.Column("invoice_number", sa.String(100), nullable=True))
    op.add_column("purchase_history", sa.Column("recorded_by_user_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("users.id", ondelete="SET NULL"), nullable=True))
