...
import { fetchTableById, updateTable } from "@/services/table";
import {
  tableUpdateSchema,
  TableUpdateFormValues,
...
export default function EditTablePage({
  ...
  const {
    ...
    queryFn: async () => {
      ...
      return fetchTableById(token, tableId);
    },
    enabled: !!isSignedIn && canManage && !!tableId,
  });
  ...
  const updateMutation = useMutation({
    mutationFn: async (values: TableUpdateFormValues) => {
      const token = await getToken();
      if (!token) throw new Error("Unauthenticated");
      return updateTable(token, tableId, values);
    },
    onSuccess: () => {
      router.push("/tables");
    },
    onError: (err: Error) => {
      setErrorMsg(err.message || "Failed to update dining table.");
    },
  });

  const onSubmit = (values: TableUpdateFormValues) => {
    setErrorMsg(null);
    updateMutation.mutate(values);
  };

  if (!isAuthLoaded || isRbacLoading || isTableLoading) {
    return (
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-6 flex flex-col items-center justify-center">
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 max-w-md w-full text-center space-y-4">
          ...
          <p className="text-sm text-slate-400">
            ...
            <strong className="text-orange-400">Manager</strong> roles can edit dining tables.
          </p>
          <Link
            href="/tables"
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-slate-100 p-6 flex flex-col items-center justify-center">
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 max-w-md w-full text-center space-y-4">
          ...
          <h1 className="text-xl font-bold text-slate-100">Dining Table Not Found</h1>
          <p className="text-sm text-slate-400">
            {(tableError as Error)?.message || "Could not find the requested dining table."}
          </p>
          <Link
            href="/tables"
            className="inline-flex items-center gap-2 px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 text-sm font-semibold rounded-xl transition-colors"
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="border-b border-slate-800 pb-5">
          ...
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-slate-50 flex items-center gap-3">
            <Edit2 className="w-7 h-7 text-orange-500" /> Edit Dining Table
          </h1>
          <p className="text-sm text-slate-400 mt-1">
            Update table number, seat capacity, status, or description.
          </p>
        </div>
